...
class APIClient(HTTPClientMixin):
    ...
    def __init__(self, base_url: str, username: str = None, password: str = None, retries: int = 3):
        ...
        self.client = httpx.Client()   # No basic auth; token headers injected per-request
    ...
    def request(self, method: str, endpoint: str, **kwargs) -> Dict[str, Any]:
        ...
        kwargs["headers"] = {**self.auth_manager.get_headers(self._auth_base_url), **existing_headers}
        try:
            response = self._do_request(method, url, **kwargs)
        except AirflowAuthError:
            # Token likely expired — refresh and retry once
            kwargs["headers"] = {**self.auth_manager.get_headers(self._auth_base_url, force_refresh=True), **existing_headers}
            response = self._do_request(method, url, **kwargs)
